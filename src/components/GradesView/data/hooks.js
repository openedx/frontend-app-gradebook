 ...
 * The currently-selected assignment's data (label/type/id, from the grades
 * results), or undefined when no assignment is selected.
 */
export const useSelectedAssignmentData = () => {
  const { results } = useGradesData();
  const { assignment: assignmentId } = useFilters();
  return useMemo(
    () => relevantAssignmentDataFromResults(results, assignmentId),
 ...
 */
export const useSelectedAssignmentLabel = () => useSelectedAssignmentData()?.label;
 ...
 * filters. Always invalidates (the base-page cache entry may still be fresh, and
 * a fresh entry is served as-is when the key flips back to it); when on a
 * prev/next page, also clears the pagination cursor.
 */
